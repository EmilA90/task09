...
function App() {
  ...
  useEffect(() => {
    setSearchResults(pizzaList.filter(pizza =>
      pizza.name.toLowerCase().includes(inputText.toLowerCase())
    ));
  }, [inputText]);

 
  const handleAddPizza = (item) => {
    setSelectedPizzas(preSelectedPizzas => [...preSelectedPizzas, item])
     
     let newCounter = selectedPizzas.length + 1;
     let newSum = selectedPizzas.reduce((total, pizza) => total + pizza.price, 0) + item.price;
  ...
  const handleRemovePizza = (id) => {
    setSelectedPizzas(preSelectedPizzas => preSelectedPizzas.filter(pizza => pizza.id !== id));
  
    let newCounter = counter - 1;
    let newSum = selectedPizzas.reduce((total, pizza) => total + pizza.price, 0) - selectedPizzas.find(pizza => pizza.id === id).price;
  ...
  return (
    <div className="App">
      <div className="search-input input-group mb-3">
        <input type="text" className="form-control" placeholder="Search" aria-label="Search" aria-describedby="button-addon2" onChange={e => setInputText(e.target.value)} value={inputText} />
      ...
      <div className="body-wrapper">
            ...
            <div className="pizza-card-box card mb-3" style={{ maxWidth: '540px' }} key={item.id}>
              <div className="row g-0">
                <div className="col-md-4">
                  ...
                  <button onClick={() => handleAddPizza(item)} className='add-btn btn btn-primary'>Add</button>
                </div>
                <div className="col-md-8">
                  <div className="card-body">
                    <h5 className="card-title">{item.name}</h5>
                    <p className="card-text">{item.dsc}</p>
                    <p className="card-text"><small className="text-muted">Price: {item.price} USD</small></p>
        ...
        <Pizzabasket selectedPizzas={selectedPizzas} counter={counter} sum={sum} handleRemovePizza={handleRemovePizza}/>
      </div>
    </div>
  );
}
